...
if __name__ == '__main__':
	...
	for line in realdata_GSEA:
		if line.startswith('chrom') or line.startswith('#'):
			idx_pval_bonf_realdata = line.strip().split().index("pval_Bonferroni")
...
#		formatted_line = [(x) for x in line.strip().split()]
		key = (int(formatted_line[0]), int(formatted_line[1]), int(formatted_line[2]))
		for i in xrange(3):
	...
	# add the rank of each window w.r.t pval_Bonf to dict_out
	list_ranks = deque(stats.rankdata([vals[idx_pval_bonf_realdata] for win,vals in dict_out.iteritems()]))
	for win in dict_out:
	...
	for f in list_simulation_GSEAfiles:
		...
		for line in f_obj:
			if line.startswith('chrom') or line.startswith('#'):
				idx_pval_bonf_sim = line.strip().split().index("pval_Bonferroni")
				continue	

			formatted_line = [float(x) for x in line.strip().split()]
			sim_key = (int(formatted_line[0]), int(formatted_line[1]), int(formatted_line[2]))

			# store Bonf_p of each window for rank calculation
			dict_win_BonfP[sim_key] = [formatted_line[idx_pval_bonf_sim]]
			...
			if sim_key in dict_GSEA_vals:
				...
				if formatted_line[idx_pval_bonf_sim] <= 0.05:
					dict_GSEA_vals[sim_key][2] += 1
					count_sigs += 1
				# for each window, count # simulations with Bonf p <= observed Bonf p
				if formatted_line[idx_pval_bonf_sim] <= dict_out[sim_key][idx_pval_bonf_realdata]:
