...
if __name__ == '__main__':
	...
	for window in vcf_chrom_iter(vcf, popsize_arr, popIndicator_arr, dict_fnames_bytemap, windowSize, stepSize):
	...
	for idx, bedCoords in enumerate(iter_split(l_bedCoords, num_batch)):
		out_fnameBED = out_fname_prefix + "chr%s_batch%s.bed" % (chromID, idx)
	...
	for idx, lsub_dict_chromFreq in enumerate(iter_split(l_dict_chromFreq, num_batch)):
		out_chromFreqDict = out_fname_prefix + "chr%s_batch%s.chromFreqDict_pickle.gz" % (chromID, idx)
		new_dict_chromFreq = {}
	...
	if idx < num_batch:
		for i in range(idx+1, num_batch):
			out_fnameBED = out_fname_prefix + "chr%s_batch%s.bed" % (chromID, i)
			with open(out_fnameBED, "a") as fout:
				fout.write("")

			out_chromFreqDict = out_fname_prefix + "chr%s_batch%s.chromFreqDict_pickle.gz" % (chromID, i)
			with gzip.open(out_chromFreqDict, "wb") as fout:
				pickle.dump([], fout)
